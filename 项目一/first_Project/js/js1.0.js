    ...
    window.q = {
        ...
        fetchComputedStyle : function (obj, property) {
            if (window.getComputedStyle) {
                property = property.replace(/([A-Z])/g, function (match, q1) {
                    return "-" + q1.toLowerCase();
                });
                return window.getComputedStyle(obj)[property];
            } else {
                property = property.replace(/-([a-z])/g, function (match, q1) {
                    return q1.toUpperCase();
                });
        ...
        hasClass : function(ele, cls) {
            ...
            if (ele.classList) {
                ...
                return ele.className.match(new RegExp('(\\s|^)' + cls + '(\\s|q)'));
            }
        },
        ...
        compile : function (templateString, dictionary) {
            return templateString.replace(/\%([0-9A-Za-z_]+)\%/g, function (match, q1) {
                return dictionary[q1];
            });
        },
        //9.剪切板
