...
(function () {
    ...
    var doGoodsCategory = function(data){
            q.each(data.data, function(item){
                var modelString=
                    "<li>" +
    ...
    var getGoodsList = function (item,index) {
        var cat_id = q.getQueryString("cat_id");
        q.get(
            ucshop.getGoodsAddress(),
            {"cat_id": cat_id, "page":1, "pagesize":9},
            function (error,d) {
                if (error) {
                    console.log(error);
                    return;
                ...
                if (q.isJSON(d)) {
                    ...
                    if (dataJSON.code == ucshop.CODE.success) {
                        q.each(dataJSON.data, function (item) {
                            var good = q.compile(shop_arrange_template, item);
    ...
    ssBtn.onclick = function () {
        if (!isshow) {
            ...
            q.animate(input1, {left: 0}, 500, "CircEaseOut");
            isshow = true;
        } else {
            q.animate(input1, {left: 260}, 500, "CircEaseOut");
            isshow = false;
        }
    };
    //=======================================================轮播图
    ...
    var doBanner = function (error, d) {
        ...
        if (q.isJSON(d)) {
            ...
            if (dataJSON.code == ucshop.CODE.success) {
                q.each(dataJSON.data, function (item, index) {
                    if (index == 0) {
    ...
    var loadCarousel = function () {
        // var loadCarousel = function (左按钮class，右按钮class，图片数组，圆点儿数组) {
        ...
        $(".rightBtn").click(function () {
            idx++;
            if (idx > $(".imagesList ul li").length - 1) {
                idx = 0;
            }
            $(".imagesList ul li:eq(" + idx + ")").addClass("current").siblings()
                .removeClass("current");
            $(".circles ol li:eq(" + idx + ")").addClass("active").siblings()
                .removeClass("active");
        });
    ...
    //校园点儿
    $("#circles").mousemove(function () {
        $(this).addClass("current").siblings().removeClass("current");
        var index = $(this).index();
        $(".imagesList ul li:eq(" + index + ")").addClass("current").siblings()
            .removeClass("current");
        idx=index;
    });
    // =====================================================手指动画
    ...
    ofingerBox.onmouseover = function () {
        if (!lock)return;
        lock = false;
        q.animate(ofinger_box, {"left": -20}, 500, "Linear", function () {
            q.animate(ofinger_box, {"left": 0}, 500, "Linear", function () {
                q.animate(ofinger_box, {"left": -20}, 500, "Linear", function () {
                    q.animate(ofinger_box, {"left": 0}, 500, "Linear");
                });
            });
        });
        setTimeout(function () {
...
//============================================获取商品列表===========
        ...
        q.get(
            ...
            function (error, d) {
                ...
                if (q.isJSON(d)) {
                    ...
                    if (dataJSON.code == ucshop.CODE.success) {
                        q.each(dataJSON.data, function (item) {
                            var sLi = q.compile(shop_arrange_template, item);
    ...
    window.onload = function(){
        ...
        ucshop.checkLogin(function(){
            q.getElementsByClassName('login')[0].style.display = 'none';
            q.getElementsByClassName('userinfo')[0].style.display = 'block';
            q.getElementsByClassName('username')[0].innerHTML = localStorage.getItem("username");
        }, function(){
            q.getElementsByClassName('login')[0].style.display = 'block';
            q.getElementsByClassName('userinfo')[0].style.display = 'none';
        });
        //设置退出按钮的事件
    ...
    function scrollAnimate(terget,time){
        ...
        var timer = setInterval(function(){
            ...
            document.body.scrollTop
                ...
                = q.Tween.Elastic.easeIn(frameNumber, start, distance, maxFrame);
         },interval)
    }
